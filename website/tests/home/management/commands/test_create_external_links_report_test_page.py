"""Tests for home/management/commands/create_external_links_report_test_page.py"""

import pytest
from io import StringIO
from unittest.mock import MagicMock, patch, call
from home.management.commands.create_external_links_report_test_page import Command
...
class TestCreatePage:
    ...
    def _was_page_created(self):
        page = None
        try:
            page = Page.objects.get(slug="external-links-report-test")
        except Page.DoesNotExist:
            page = None

        return page is not None

    def test_page_delete_when_not_exists_logs_message(self, root_page):
        with patch(
            "home.management.commands.pages.external_links_report_test_page.logger"
        ) as mock_logger:
            cmd = self._make_cmd()
        ...
        assert (
            "Test page with slug 'external-links-report-test' does not exist."
    ...
    def test_page_creation_with_root_page_existing_creates_page(self, root_page):
        ...
        assert mock_logger.info.call_args_list == [
            call("Creating the 'External Links Report Test Page' page."),
            call("Created the 'External Links Report Test Page' page."),
        ]

    def test_page_creation_without_root_page_existing_does_not_create_page(self):
        with patch(
            "home.management.commands.pages.external_links_report_test_page.logger"
        ) as mock_logger:
            cmd = self._make_cmd()
    ...
    def test_page_creation_twice_with_root_page_existing_creates_page(self, root_page):
        ...
        assert mock_logger.info.call_args_list == [
            call("Creating the 'External Links Report Test Page' page."),
            call("Created the 'External Links Report Test Page' page."),
            call("- External Links Report Test Page page already exists."),
        ]
    ...
    def test_page_creation_with_root_page_existing_then_deleting(self, root_page):
        ...
        assert mock_logger.info.call_args_list == [
            call("Creating the 'External Links Report Test Page' page."),
            call("Created the 'External Links Report Test Page' page."),
            call("Deleted test page with slug 'external-links-report-test'."),
        ]
