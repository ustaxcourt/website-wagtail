# Generated by Django 5.2.15 on 2026-08-05 13:24
#
# Also backfills the new CardTilesBlock back button fields on existing
# "card_tiles" blocks so the (single, shared) back button shows up by
# default for card tile sets that were created before the field existed.
#
# The back button belongs to the CardTilesBlock container (not to
# individual tiles) - one button governs the whole set of tiles (A, B,
# C, ...) and is shown whenever any tile is selected (e.g. p/a, p/b) and
# hidden on the default view showing all tiles. It only has a visible
# effect when at least one tile links to an "anchor_page" (the only link
# type that keeps the user on the same page/URL param instead of
# navigating away), so only card_tiles blocks with at least one such
# tile are backfilled.
#
# Uses the historical model, so this will catch live pages but not draft
# revisions.
...
def backfill_back_button(apps, schema_editor):
    ...
    for page in EnhancedStandardPage.objects.all():
        ...
        if changed:
            page.save()
            logger.info(
                f"0132: backfilled back button on EnhancedStandardPage id={page.pk}.: {page.title}"
            )
...
class Migration(migrations.Migration):
    dependencies = [
        ("home", "0131_alter_enhancedstandardpage_body"),
    ]
        ...
        migrations.AlterField(
            ...
            field=wagtail.fields.StreamField(
                ...
                block_lookup={
                    ...
                    117: ("wagtail.blocks.PageChooserBlock", (), {}),
                    118: ("wagtail.blocks.URLBlock", (), {}),
                    119: (
                        "wagtail.blocks.StreamBlock",
                        [[("related_page", 117), ("external_url", 118)]],
                        {
                    ...
                    132: (
                        ...
                        [
                            [
                                ("tiles", 128),
                                ("default_content", 129),
                                ("show_back_button", 130),
                                ("back_button_text", 131),
                            ]
                        ],
                        {},
                    ),
