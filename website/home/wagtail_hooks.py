...
class ConditionalRoleSwitcherMenuItem(MenuItem):
    ...
    def is_shown(self, request):
        print(f"\n[ConditionalRoleSwitcherMenuItem.is_shown] Checking for user: {request.user.username}")

        # Condition 1: The user is currently a superuser (in their DB record)
        is_current_superuser = request.user.is_authenticated and request.user.is_superuser
