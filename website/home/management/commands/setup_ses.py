...
def verify_domain_and_get_tokens(ses_client):
    ...
    domain = input("Enter the domain you want to verify (e.g., yourdomain.com): ").strip().lower()
    if not domain:
    ...
    try:
        ...
        if not dkim_tokens:
            print("Could not retrieve DKIM tokens. The domain might already be verified.")
            return domain, []
...
def add_records_to_route53(route53_client, domain, dkim_tokens):
    ...
    try:
        ...
        if not hosted_zone_id:
            ...
            print("   Please ensure the domain is managed by Route 53 in this AWS account.")
            return False
