...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        page_creation_methods = [
            self.create_case_related_forms_page,
        ]

        try:
            home_page = Page.objects.get(slug="home")
        except Page.DoesNotExist:
            self.stdout.write(self.style.ERROR("Home page does not exist."))
            return

        for method in page_creation_methods:
            method(home_page)

        for form in forms_data:
            self.create_form_entry(form)

    def create_case_related_forms_page(self, home_page):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Successfully created the '{title}' page.")
        )
    ...
    def create_form_entry(self, formData):
        try:
            ...
            self.stdout.write(
                self.style.ERROR("Parent page 'Case Related Forms' does not exist.")
            )
            return

        # Check if the form already exists
        if CaseRelatedFormsEntry.objects.filter(formName=formData["formName"]).exists():
            self.stdout.write(self.style.SUCCESS("Form entry already exists."))
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully created form entry: {formData['formName']}"
            )
        )
