...
def associate_existing_user_by_email(
    ...
    try:
        ...
        print(
            f"SSO login for {email}: Associating with existing user '{existing_user.username}'."
        )
        return {"user": existing_user, "is_new": False}
    except User.DoesNotExist:
        print(
            f"SSO login for {email}: No existing user found with this email. Pipeline will continue."
        )
        pass

    except User.MultipleObjectsReturned:
        print(
            f"CRITICAL: Multiple users found with email {email}. Manual intervention required."
        )
        return None
