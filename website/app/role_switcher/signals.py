...
def revert_role_on_logout(sender, request, user, **kwargs):
    ...
    """
    if not user: # User might be None if logout is part of user deletion or anonymous logout
        return
    ...
    try:
        ...
        was_originally_superuser = request.session.get(SESSION_ORIGINAL_IS_SUPERUSER_KEY, False)
        ...
        if is_assuming_role and was_originally_superuser:
            ...
            user_to_revert.is_superuser = request.session.get(SESSION_ORIGINAL_IS_SUPERUSER_KEY, False)
            user_to_revert.is_staff = request.session.get(SESSION_ORIGINAL_IS_STAFF_KEY, False)
            ...
            logger.info(f"User '{user_to_revert.username}' (ID: {user_to_revert.pk}) was automatically reverted from an assumed role upon logout.")
        ...
        logger.error(f"User with PK {user.pk} not found during automatic role revert on logout.")
    except Exception as e:
        # Log any other errors during this process, but don't prevent logout
        logger.error(f"Error during automatic role revert for user '{user.username}' (ID: {user.pk}) on logout: {e}", exc_info=True)
