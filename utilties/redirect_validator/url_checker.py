...
def check_urls(base_url, input_csv_path, output_csv_path):
    ...
    try:
        ...
        with open(input_csv_path, 'r', encoding='utf-8-sig') as infile, \
            ...
            writer.writerow(['Result', 'Final Status Code', 'Original Relative URL', 'Full URL Checked', 'Final Destination URL', 'Redirect Chain'])
            ...
            for i, row in enumerate(reader):
                ...
                try:
                    ...
                    writer.writerow([emoji, status_code, relative_url, full_url, final_url, redirect_chain])
                    print(f"{emoji} SUCCESS: {full_url} -> {final_url} ({status_code})")

                except requests.exceptions.RequestException as e:
                    # Handle network errors, timeouts, etc.
                    error_message = str(e)
                    writer.writerow(['❗', 'N/A', relative_url, full_url, 'Error', error_message])
                    print(f"❗ ERROR: {full_url} - {error_message}", file=sys.stderr)

    except FileNotFoundError:
        print(f"FATAL ERROR: The input file was not found at '{input_csv_path}'", file=sys.stderr)
        return
...
def main():
    ...
    parser.add_argument('base_url', help="The base URL to test against (e.g., 'https://www.example.com').")
    parser.add_argument('input_csv', help="Path to the input CSV file. The file should contain one relative URL per row in the first column.")
